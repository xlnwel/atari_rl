...
from utility.tf_utils import get_norm, wrap_layer
...
class Network(Module):
    ...
    def _conv_net(self, x, name=None):
        ...
        return wrap_layer(name, lambda: net(x))
...
class Rainbow(Network):
    ...
    def _build_graph(self):    
        ...
        # [B, 1, N], [B, 1, N]
        self.dist = self._c51_action_value(self.action, dist, 'logits')
        self.dist_next_target = self._c51_action_value(next_action, dist_next_target, 'dist_next_target')
        self.Q = self._c51_action_value(self.best_action, Qs, 'best_Q')     # for tensorflow bookkeeping
    ...
    def _c51_action_value(self, action, values, name):
        with tf.name_scope(name):
            ...
            # value = tf.gather_nd(values, action)

        return value

class Rainbow_IQN(Network):
    """ Interface """
    def __init__(self, 
                 name, 
                 args, 
